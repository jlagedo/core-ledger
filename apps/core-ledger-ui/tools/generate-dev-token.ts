  ...
  try {
    ...
    for (const [role, user] of Object.entries(MOCK_USERS)) {
      ...
      if (!mockUser.oidcUserData?.sub || !mockUser.permissions) {
        console.warn(`⚠️  Skipping user '${role}' - missing required fields`);
        continue;
      }
    ...
    console.error('❌ Failed to load mock-users.ts:', error);
    throw error;
  }
}
...
async function main(): Promise<void> {
  ...
  console.log('🔍 Loading mock users from mock-users.ts...\n');

  // Load user configurations by importing the module
  const userConfigs = await loadMockUsers();

  if (userConfigs.length === 0) {
    console.error('❌ No user configurations found in mock-users.ts');
    process.exit(1);
  }
  ...
  console.log(`📋 Found ${userConfigs.length} user(s) in MOCK_USERS`);
  console.log(`🔐 Generating JWT tokens for ${usersToProcess.length} user(s)...\n`);
  ...
  console.log(`✅ Updated ${mockTokensPath}`);
  console.log('\n✨ Done! Mock tokens have been regenerated.');
  console.log('💡 Restart your dev server to use the new tokens.\n');
}
